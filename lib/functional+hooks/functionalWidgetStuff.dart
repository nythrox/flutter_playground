...
class PropfulwidgetCallbackClass<T> {
  final PropfulFunctionalWidget<T> widget;
  PropfulFunctionalWidget<T> call(T props, {Key key}) => widget;
  const PropfulwidgetCallbackClass(this.widget);
}

typedef PropfulWidgetFunction<T> = Widget
    Function(BuildContext context, T props, {Key key});

typedef PropfulWidgetCallback<T> = PropfulFunctionalWidget<T> Function(T props,
    {Key key});

PropfulWidgetCallback<T> propfulFunctionalWidget<T>(
        PropfulWidgetFunction<T> builder) =>
    (T props, {Key key}) => new PropfulFunctionalWidget<T>(
          builder,
          props,
          key: key,
        );
...
class _FunctionalWidgetState<T> extends State<PropfulFunctionalWidget<T>> {
  @override
...
class PropfulFunctionalWidget<T> extends _FunctionalWidget<T> {
  /// Initializes [key] for subclasses.
  const PropfulFunctionalWidget(
    this.builder,
    this.props, {
    Key key,
